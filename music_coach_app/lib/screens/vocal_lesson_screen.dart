...
class VocalLessonScreen extends StatefulWidget {
  final VoidCallback? onBackPressed;

  const VocalLessonScreen({super.key, this.onBackPressed});

  @override
  State<VocalLessonScreen> createState() => _VocalLessonScreenState();
}
...
class _VocalLessonScreenState extends State<VocalLessonScreen>
  ...
  // Generate all lessons placeholders dynamically based on backend data
  ...
  Future<void> _loadLessons() async {
    try {
      final modules = await LessonService.fetchModules(instrumentType: 'vocals');
  ...
  void _onLessonStart(int placeholderIndex, LessonItem? backendLesson, bool isLocked, {bool isJump = false, int? targetLevel, int? targetLessonIndex, List<LessonModule>? allModules}) {
    if (backendLesson != null && (!isLocked || isJump)) { 
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => VocalLessonPerformScreen(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          ...
          backgroundColor: Color(0xFFE93B81),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) {
      return const Scaffold(
        ...
        body: Center(child: CircularProgressIndicator(color: Color(0xFFE93B81))),
      );
    }
    ...
    if (_needsCalibration) {
      return PopScope(
        ...
        child: VocalPitchCalibrationScreen(
          onCalibrationComplete: (double pitchHz) async {
             ...
             if (mounted) {
                setState(() {
                   ...
                   if (_profile != null) _profile!['natural_pitch'] = pitchHz;
                });
             }
          },
        ),
